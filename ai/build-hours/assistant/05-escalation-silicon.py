...
def escalate_to_human(summary):
    ...
    print(colored("Escalating to human agent...", "red"))
    print("\n=== Escalation Report ===")
    print(f"Summary: {summary}")
    print("=========================\n")
    exit()

tools = [look_up_item, execute_refund, escalate_to_human]
client = openai.OpenAI(api_key="sk-nsswwpfvuompvcqcseqsfbhjysigfiiybqyeznikustuhucq", base_url="https://api.siliconflow.cn/v1")  
...
def chat_with_bot():
    ...
    while True:
        try:
            ...
            print(colored("\n对话中断，再见！",'magenta'))
            break
        run_full_turn(messages, user_input)


def run_full_turn(messages, user_input):
    tool_schemas = [function_to_schema(func) for func in tools]
    messages.append({"role": "user", "content": user_input})
    while True:
        response = client.chat.completions.create(model='Qwen/Qwen2.5-72B-Instruct', messages=messages, tools=tool_schemas)
        msg = response.choices[0].message
        messages.append(msg)

        if msg.content:
            print(colored("客服:", 'cyan'), msg.content)
